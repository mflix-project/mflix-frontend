...
export default function MovieDetail() {
  ...
  useEffect(() => {
    fetch(`https://mflix-james.herokuapp.com/api/movies/${id}`)
      .then((res) => {
        if (!res.ok) {
          throw new Error("Could not fetch the requested movie info");
        }
        return res.json();
      })
      .then((result) => {
      ...
      .catch((err) => {
        console.error(`Something is wrong while fetching movie info - ${err}`);
      })
    ...
    <>
      ...
      <Card className="p-4" style={{ width: "70%" }}>
        ...
        <Card.Body>
          ...
          <Card.Text>{data.countries.join(", ")}</Card.Text>
