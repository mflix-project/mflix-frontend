...
export default function TheaterDetail() {
  ...
  useEffect(() => {
    fetch(`https://mflix-james.herokuapp.com/api/theaters/${id2}`)
      .then((res) => {
        if (!res.ok) {
          throw new Error("Could not fetch the requested theater info");
        }
        return res.json();
      })
      .then((result) => {
      ...
      .catch((err) => {
        console.error(`Something is wrong while fetching city info - ${err}`);
      })
      ...
      <div className="mapStyle">
        <Container className="p-3">
          ...
          >
            ...
            >
              <Popup>
                {data.location.address.street1}, {data.location.address.city},{" "}
                {data.location.address.state}
              </Popup>
            </Marker>
          </MapContainer>
